...
def run_models(list_of_models_to_run, dataset_dir, file_list_path, score_dir_base, metrics_dir_base, save_metrics, hp_dict, seed_value, process_id):
    # Configure process-specific logging first
    process_logger = logging.getLogger(f"process_{process_id}")
    process_logger.setLevel(logging.INFO)
    
    # Make sure handlers are cleared (in case of reuse)
    if process_logger.handlers:
        for handler in process_logger.handlers:
            process_logger.removeHandler(handler)
    
    # Create a console handler for this process
    console = logging.StreamHandler()
    console.setFormatter(logging.Formatter('%(asctime)s - Process %(process)d - %(levelname)s - %(message)s'))
    process_logger.addHandler(console)

    # starting timer
    process_start_time = time.time()

    # starting logging with the configured logger
    process_logger.info(f"--- Process {process_id} (PID: {os.getpid()}) started, handling models: {list_of_models_to_run} ---")
    ...
    try:
        ...
        process_logger.error(f"[Process {process_id}] File list not found at {file_list_path}. Exiting process.")
        return
    except Exception as e:
        process_logger.error(f"[Process {process_id}] Error loading file list: {e}. Exiting process.")
        return
    ...
    for model_name in list_of_models_to_run:
        ...
        try:
            ...
            # Create a model-specific logger that is separate from the process logger
            model_logger = logging.getLogger(f"process_{process_id}.model_{model_name}")
            model_logger.setLevel(logging.INFO)
            
            # Clear any existing handlers
            if model_logger.handlers:
            ...
            model_logger.addHandler(console)
            
            # Make sure the logger propagates to parent loggers (including the process logger)
            model_logger.propagate = False

            model_logger.info(f"--- [Model: {model_name}] Starting processing (Process {process_id}) ---")
            model_logger.info(f"[Model: {model_name}] Using arguments: dataset_dir={dataset_dir}, file_list_path={file_list_path}, score_dir_base={score_dir_base}, metrics_dir_base={metrics_dir_base}, save_metrics={save_metrics}, seed_value={seed_value}")
            
            # Check if the model name exists in the dictionary before accessing it
            if model_name not in hp_dict:
                model_logger.error(f"[Model: {model_name}] Optimal hyperparameters not found. Skipping this model.")
                if file_handler:
            ...
            model_logger.info(f'[Model: {model_name}] Optimal_Det_HP: {Optimal_Det_HP}')
            ...
            for filename in file_list:
                ...
                if os.path.exists(score_file_path):
                    model_logger.info(f"[Model: {model_name}] Score file already exists, skipping: {score_file_path}")
                    continue

                model_logger.info(f'[Model: {model_name}] Processing: {filename}')
                ...
                try:
                    ...
                    try:
                        ...
                        model_logger.error(f"[Model: {model_name}] Could not determine train_index from filename {filename}: {e}. Skipping file.")
                        continue
                    ...
                    if model_name in Semisupervise_AD_Pool:
                        ...
                        model_logger.error(f"[Model: {model_name}] {model_name} is not defined in known supervised/unsupervised pools.")
                        continue
                    ...
                    if isinstance(output, np.ndarray):
                        model_logger.info(f'[Model: {model_name}] Success at {filename} | Time cost: {run_time:.3f}s at length {len(label)}')
                        np.save(score_file_path, output)
                    else:
                        model_logger.error(f'[Model: {model_name}] At {filename}: {output}')
                        continue

                    if save_metrics:
                        try:
                            if len(output) != len(label):
                                model_logger.warning(f"[Model: {model_name}] Output length ({len(output)}) and label length ({len(label)}) mismatch for {filename}. Padding or truncating output.")
                                if len(output) > len(label):
                                ...
                                model_logger.info(f"[Model: {model_name}] Adjusted output length to {len(output)}.")
                            ...
                            if col_w is None:
                                if evaluation_result:
                                     ...
                                     model_logger.error(f"[Model: {model_name}] Cannot determine column names because evaluation_result is empty for {filename}.")
                                     continue

                            if col_w is None:
                                model_logger.error(f"[Model: {model_name}] Column names not set, cannot append metrics for {filename}.")
                                continue
                            ...
                            model_logger.error(f"[Model: {model_name}] Error calculating metrics or appending to CSV for {filename}: {e}", exc_info=True)
                            model_logger.error(f"[Model: {model_name}] Output shape: {output.shape if isinstance(output, np.ndarray) else 'N/A'}, Label shape: {label.shape}, Sliding window: {slidingWindow}")

                except FileNotFoundError:
                    model_logger.error(f"[Model: {model_name}] Dataset file not found: {file_path}")
                except Exception as e:
                    model_logger.error(f"[Model: {model_name}] Error processing file {filename}: {e}", exc_info=True)
            # --- End of file loop ---

        except ImportError as e:
             model_logger.error(f"[Model: {model_name}] Import error: {e}. Check TSB_AD installation/dependencies.")
        except KeyError as e:
             model_logger.error(f"[Model: {model_name}] KeyError: {e}. Missing from HP dict?")
        except Exception as e:
            model_logger.error(f"[Model: {model_name}] An unexpected error occurred: {e}", exc_info=True)
        finally:
            if file_handler and model_logger:
                model_logger.removeHandler(file_handler)
                file_handler.close()
            model_end_time = time.time()
            process_logger.info(f"--- [Model: {model_name}] Finished processing (Process {process_id}). Duration: {(model_end_time - model_start_time):.2f}s ---")
    # --- End of model loop ---

    process_end_time = time.time()
    process_logger.info(f"--- Process {process_id} (PID: {os.getpid()}) finished. Total Duration: {(process_end_time - process_start_time)/60:.2f} min ---")



if __name__ == '__main__':
    
    # Set up root logger for main process only
    logging.basicConfig(level=logging.INFO, 
                        format='%(asctime)s - %(levelname)s - %(message)s',
                        handlers=[logging.StreamHandler()])
    
    # When using multiprocessing with logging, this helps:
    ...
    model_list_1 = ['PCA'] #DONE: 'IForest', 'LOF', 'HBOS', 'OCSVM', 'MCD', 'KNN' PROBLEM:  'PCA'
    model_list_2 = ['RobustPCA'] #DONE: 'KMeansAD', 'KShapeAD', 'COPOD', 'CBLOF', PROBLEM:  'EIF'
    model_list_3 = ['LSTMAD', 'TranAD', 'OmniAnomaly', 'USAD', 'FITS'] #DONE: 'AnomalyTransformer', 'AutoEncoder', 'CNN',

    
    list_of_model_lists = [model_list_1]
