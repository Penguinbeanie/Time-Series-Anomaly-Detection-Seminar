...
if __name__ == '__main__':
     # --- Configuration ---
    SEED = 2024
    DATASET_DIR = r'C:\Users\Kai\Documents\Time_Series_Anomaly_Detection\Time-Series-Anomaly-Detection-Seminar\TSB-AD\Datasets\TSB-AD-U-S'
    FILE_LIST_PATH = r'C:\Users\Kai\Documents\Time_Series_Anomaly_Detection\Time-Series-Anomaly-Detection-Seminar\TSB-AD\Datasets\File_List\self_dataset.csv'
    SCORE_DIR_BASE = 'eval/self/score/uni/'
    METRICS_DIR_BASE = 'eval/self/metrics/uni/'
    SAVE_METRICS = True

    # model lists for each process
    model_list_1 = ['AnomalyTransformer', 'MOMENT_ZS', 'FFT', 'SR', 'Sub_IForest', 'IForest', 'LOF', 'Sub_LOF', 'POLY', 'MatrixProfile', 'Sub_PCA', 
    ...
    list_of_model_lists = [model_list_1, model_list_2, model_list_3]
    ...
    try:
        ...
        if 'Optimal_Uni_algo_HP_dict' not in globals():
             raise NameError("Optimal_Uni_algo_HP_dict is not defined/imported globally.")
    ...
    for i, current_model_list in enumerate(list_of_model_lists):
        process_id = i + 1
        if not current_model_list:
            logging.warning(f"Skipping process {process_id} as its model list is empty.")
            continue # Don't start a process for an empty list
    ...
    if not processes:
        ...
        for i, p in enumerate(processes):
            original_process_id = i + 1
            p.join()
            logging.info(f"Process {original_process_id} finished.") # Log using the original ID
        
    Global_End_T = time.time()
    logging.info(f"--- All Launched Processes Finished ---")
    logging.info(f"\n\nTotal Global Running Time: {(Global_End_T - Global_Start_T) / 60:.2f} min")
