...
if __name__ == '__main__':
    ...
    Start_T = time.time()
    ## ArgumentParser
    model_list = ['MOMENT_ZS', 'FFT', 'SR', 'Sub_IForest', 'IForest', 'LOF', 'Sub_LOF', 'POLY', 'MatrixProfile', 'Sub_PCA',
    ...
    for model in model_list:
        ...
        try:
            ...
            parser.add_argument('--file_list', type=str, default=r'C:\Users\Kai\Documents\Time_Series_Anomaly_Detection\Time-Series-Anomaly-Detection-Seminar\TSB-AD\Datasets\File_List\TSB-AD-U.csv')
            ...
            for filename in file_list:
                ...
                try:
                    ...
                    if args.AD_Name in Semisupervise_AD_Pool:
                        ...
                        # This case should ideally not be reached if AD_Name is checked against Optimal_Uni_algo_HP_dict keys
                        # and assuming Pools align with the HP dict keys.
                    ...
                    if args.save:
                        logging.info("args.save is True, calculating metrics.")
                        try:
                            ...
                            if len(output) != len(label):
                                ...
                                # Example: Pad with last value or zero, or truncate
                                if len(output) > len(label):
                                    output = output[:len(label)]
                                else:
                                    # Simple padding strategy (e.g., pad with zeros or last value)
                            ...
                            logging.info(f'evaluation_result: {evaluation_result}')
                            ...
                            list_w = [0]*9 # Default values on error
            ...
            # Save metrics CSV after processing all files for the current model
            if args.save and write_csv: # Check if there's anything to write
                # Define columns based on the last successful evaluation_result or provide default names
                if evaluation_result:
                    col_w = list(evaluation_result.keys())
                else:
                    # Provide default column names if evaluation never succeeded
                    col_w = [f'metric_{i+1}' for i in range(9)] # Assuming 9 metrics
                col_w.insert(0, 'Time')
                col_w.insert(0, 'file')
                try:
                    w_csv = pd.DataFrame(write_csv, columns=col_w)
                    csv_path = os.path.join(target_dir_metrics, f"{args.AD_Name}.csv")
                    w_csv.to_csv(csv_path, index=False)
                    logging.info(f"Metrics saved to {csv_path}")
                except Exception as e:
                    logging.error(f"Failed to save metrics CSV for model {args.AD_Name}: {e}")
            elif args.save:
                 logging.warning(f"Metrics saving enabled, but no results generated or processed for model {args.AD_Name}.")
