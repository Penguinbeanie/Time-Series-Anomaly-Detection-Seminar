...
from TSB_AD.evaluation.metrics import get_metrics_and_best_threshold
...
if __name__ == '__main__':
    ...
    # 'Left_STAMPi', 'MOMENT_ZS', 'FFT', 'SR', 'Sub_IForest', 'IForest', 'LOF', 
    ...
    for model in model_list:
        ...
        try:
            ...
            parser.add_argument('--dataset_dir', type=str, default=r'C:\Users\Kai\Documents\Time_Series_Anomaly_Detection\Time-Series-Anomaly-Detection-Seminar\TSB-AD\Datasets\TSB-AD-U-Self')
            parser.add_argument('--file_list', type=str, default=r'C:\Users\Kai\Documents\Time_Series_Anomaly_Detection\Time-Series-Anomaly-Detection-Seminar\TSB-AD\Datasets\File_List\self_dataset.csv')
            parser.add_argument('--score_dir', type=str, default='eval/self/score/uni/')
            parser.add_argument('--save_dir', type=str, default='eval/self/metrics/uni/')
            parser.add_argument('--pred_dir', type=str, default='eval/self/predictions/uni/')
            ...
            # Create directories for scores, metrics, and predictions for the current model
            target_dir_score = os.path.join(args.score_dir, args.AD_Name)
            target_dir_metrics = os.path.join(args.save_dir, args.AD_Name)
            target_dir_preds = os.path.join(args.pred_dir, args.AD_Name)
            os.makedirs(target_dir_score, exist_ok=True)
            os.makedirs(target_dir_metrics, exist_ok=True)
            os.makedirs(target_dir_preds, exist_ok=True)

            # Configure file logging specifically for this model run
            log_file_path = os.path.join(target_dir_score, f'000_run_{args.AD_Name}.log')
            ...
            for filename in file_list:
                score_file_path = os.path.join(target_dir_score, f"{args.AD_Name}_{filename.split('.')[0]}.npy")
                ...
                try:
                    ...
                    if args.save:
                        ...
                        logging.info("Calculating metrics, determining optimal threshold, and generating predictions.")
                        try:
                            ...
                            # Call the new function to get metrics, threshold, and predictions
                            evaluation_result, optimal_threshold, binary_predictions = get_metrics_and_best_threshold(
                                output, label, slidingWindow=slidingWindow
                            )
                            logging.info(f'Evaluation result: {evaluation_result}')
                            if optimal_threshold is not None:
                                logging.info(f'Optimal threshold (for PointF1) for {filename}: {optimal_threshold:.6f}')
                            else:
                                logging.warning(f'Optimal threshold for {filename} was not determined (returned None).')

                            # Save binary predictions
                            pred_file_name = f"PRED_{args.AD_Name}_{filename.split('.')[0]}.npy"
                            pred_file_path = os.path.join(target_dir_preds, pred_file_name)
                            np.save(pred_file_path, binary_predictions)
                            logging.info(f"Saved binary predictions to {pred_file_path}")
                            ...
                            logging.error(f"Error calculating metrics, saving predictions, or appending to CSV for {filename}: {e}", exc_info=True)
                            ...
                            # Ensure evaluation_result is a dict for CSV part, even if partial success/failure
                            if 'evaluation_result' not in locals() or not isinstance(evaluation_result, dict):
                                evaluation_result = {} 
